...
def read_2d_list_from_file(file_name):
        with open(file_name, 'r') as file:
            # Read lines from the file
            lines = file.readlines()

            # Create a 2D list to store integers
            two_d_list = []
            ...
            for line in lines:
                ...
                # Append the list of integers to the 2D list
                two_d_list.append(numbers)
...
with open('main_file.txt', 'r') as file:
    ...
    if len(lines) >= 2:  # Check if there are at least two lines in the file
        second_line = lines[1]  # Index 1 corresponds to the second line (indexing starts from 0)
        if os.path.exists(second_line):
            c = 1
...
value = 0
if c == 1 :
    defaultgrid = read_2d_list_from_file(second_line)
if c == 0 :
    defaultgrid = grid
...
pygame.quit()
if not os.path.exists(second_line):

    for row_index in range(len(defaultgrid)):
        for col_index in range(len(defaultgrid[row_index])):
            defaultgrid[row_index][col_index] = str(defaultgrid[row_index][col_index])

    data_string = '\n'.join([' '.join(line) for line in defaultgrid])
